namespace AoC2022
{
    public class Day04 : Solution
    {
        ...
        public override string SolvePart2() => pairs
            .Count(p => (p.p1Start <= p.p2Start && p.p1End >= p.p2Start) ||
                        (p.p1Start <= p.p2End   && p.p1End >= p.p2End) ||
                        (p.p2Start <= p.p1Start && p.p2End >= p.p1Start) ||
                        (p.p2Start <= p.p1End   && p.p2End >= p.p1End)).ToString();
